...
- Imports two internal packages: `internal/libver` for the `Version` constant, and `internal/versioninfo` for the `ReadVersionInfo` helper — it does **not** import the service package.
  ...
  That leaves [`config.go`](../../go-cli-config/reference/configuration.md#cmdnamecommandsconfiggo-always-present) (in the `go-cli-config` skill) as the one `commands/` file that imports `{{NAME}}` directly.
